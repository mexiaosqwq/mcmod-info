...
| 值 | 出现位置 | 说明 |
|----|---------|------|
| `not_found` | `fetch_mod_info()`, `get_mod_dependencies()` | 项目/资源不存在（404 或空响应） |
| `rate_limited` | `fetch_mod_info()` | Modrinth API 限流（HTTP 429，360 请求/小时） |
| `api_failed` | `fetch_mod_info()`, `get_mod_dependencies()` | API 请求失败/超时（网络错误） |
| `parse_failed` | `_parse_mcmod_mod_result()`, `_extract_mcmod_relationships()` | HTML 存在但解析失败 |
